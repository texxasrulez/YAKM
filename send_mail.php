...
/* Shared DB connection (submissions + rate limiting) */
$__kontact_db_creds = [
    'host' => KCFG('DATABASE_HOST',''),
    'name' => KCFG('DATABASE_NAME',''),
    'user' => KCFG('DATABASE_USER',''),
    'pass' => KCFG('DATABASE_PASS',''),
];
$pdo = null;
$db = null;
if ($__kontact_db_creds['host'] !== '' && $__kontact_db_creds['name'] !== '' && $__kontact_db_creds['user'] !== '' && class_exists('\Kontact\Database')) {
    try {
        $db = new \Kontact\Database([
            'DATABASE_HOST' => $__kontact_db_creds['host'],
            'DATABASE_NAME' => $__kontact_db_creds['name'],
            'DATABASE_USER' => $__kontact_db_creds['user'],
            'DATABASE_PASS' => $__kontact_db_creds['pass'],
        ]);
        $pdo = $db->pdo();
    } catch (\Throwable $e) {
        $db = null;
        $pdo = null;
        try { kontact_log('database.log', 'connect_failed '.$e->getMessage()); } catch (\Throwable $ignored) {}
    }
}
...
try {
    if (class_exists('\Kontact\Security') && class_exists('\Kontact\Database')) {
        ...
        if ($dbForSec && \Kontact\Security::isBlocked($dbForSec)) { deny_spam('security:isBlocked', $spammer_page); }
    }
} catch (\Throwable $e) {}
...
if ($have_tokens) {
    ...
    if (!hash_equals($calc, (string)$ksig)) {
        ...
        if (function_exists('kontact_log')) {
            kontact_log('security.log', 'kt_ksig_signature_mismatch skipping timing gate', ['category' => 'security']);
        }
    } else {
...
if ((!$pdo || !$db) && $__kontact_db_creds['host'] && $__kontact_db_creds['name'] && $__kontact_db_creds['user'] && class_exists('\Kontact\Database')) {
    try {
        $db = new \Kontact\Database([
            'DATABASE_HOST'=>$__kontact_db_creds['host'],
            'DATABASE_USER'=>$__kontact_db_creds['user'],
